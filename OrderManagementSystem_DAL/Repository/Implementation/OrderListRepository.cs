...
namespace OrderManagementSystem_DAL.Repository.Implementation
{
    public class OrderListRepository : IOrderListRepository
    {
        ...
        public async Task<ResponseModel> GetAllOrderDataByAgency(
    int agencyId,
    DateTime? fromDate,
    DateTime? toDate)
        {
            ...
            try
            {
                ...
                {
                    var from = fromDate.Value.Date; // 00:00:00
                    query = query.Where(x => x.BillDate >= from);
                }

                if (toDate.HasValue)
                {
                    var to = toDate.Value.Date.AddDays(1); // next day 00:00
        ...
        public async Task<ResponseModel> CreateOrder(AddOrderModel model)
        {
            ...
            try
            {
                ...
                {
                    _context.SalesDetails.Add(new OrderManagementSystem.Entities.SalesDetail
                    {
